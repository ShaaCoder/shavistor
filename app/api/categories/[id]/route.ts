/**
 * Category by ID/Slug API Route
 * 
 * GET /api/categories/[id] - Get individual category by id or slug with enhanced SEO data
 */
 ...
 * Retrieve individual category by id or slug with enhanced SEO metadata and products
 */
  ...
  try {
    ...
    if (!category) {
      return createErrorResponse(
        'Category not found',
        404,
        `Category with ${isObjectId ? 'ID' : 'slug'} '${id}' not found`
      );
    }
...
export async function PUT(
  ...
  return withAuth(async (request) => {
    try {
      ...
      return handleApiError(error, `PUT /api/categories/${params.id}`);
    }
  }, ['admin'])(request);
}
...
export async function DELETE(
  ...
  return withAuth(async () => {
    try {
      ...
      return handleApiError(error, `DELETE /api/categories/${params.id}`);
    }
  }, ['admin'])(request);
}
