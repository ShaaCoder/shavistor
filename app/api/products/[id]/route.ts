/**
 * Product by ID/Slug API Routes
 * 
 * GET /api/products/[id] - Get product by id or slug (param name unified)
 */
...
/**
 * GET /api/products/[id]
 * Retrieve a specific product by id or slug (value)
 */
export async function GET(
  ...
  try {
    ...
    // Find product by slug (value in params.id)
    ...
    if (!product) {
      return createErrorResponse(
        'Product not found',
        404,
        `Product with slug '${params.id}' not found`
      );
    }
    ...
    return handleApiError(error, `GET /api/products/${params.id}`);
  }
}
...
export async function PUT(
  ...
  return withAuth(async (request) => {
    try {
      ...
      const product = await Product.findOneAndUpdate({ slug: params.id }, updates, { new: true, runValidators: true })
      ...
      return handleApiError(error, `PUT /api/products/${params.id}`);
    }
  }, ['admin'])(request);
}
...
export async function DELETE(
  ...
  return withAuth(async () => {
    try {
      await connectDB();
      const deleted = await Product.findOneAndDelete({ slug: params.id });
      if (!deleted) {
        return createErrorResponse('Product not found', 404, 'Not Found');
      }
      return createSuccessResponse<{ slug: string }>({ slug: params.id }, 'Product deleted successfully');
    } catch (error) {
      return handleApiError(error, `DELETE /api/products/${params.id}`);
    }
  }, ['admin'])(request);
}
