  ...
  async createShiprocketShipment(request: CreateShipmentRequest): Promise<ShipmentResponse> {
    try {
      ...
      if (data.status === 1 || data.status_code === 1 || data.order_id || data.shipment_id) {
        ...
        // Attempt to assign AWB immediately if not provided in creation response
        let awbNumber: string | null = data.awb_code || data.airwaybill_number || null;
        let assignResponseData: any = null;
        ...
        if (!awbNumber && shipmentId) {
          try {
            ...
            const assignRes = await fetch(`${this.config.shiprocket.baseUrl}/courier/assign/awb`, {
              method: 'POST',
              headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
            ...
            assignResponseData = await assignRes.json().catch(() => ({}));
            console.log('🚚 Shiprocket AWB assignment response:', JSON.stringify(assignResponseData, null, 2));

            // Extract awb from common fields
            awbNumber = assignResponseData?.awb_code
              || assignResponseData?.awb
              || assignResponseData?.data?.awb_code
              || null;
          } catch (assignErr) {
            console.warn('⚠️ AWB assignment step failed:', assignErr);
          }
        }

        const finalAwb = awbNumber;
        const trackingUrl = finalAwb
          ? `https://shiprocket.co/tracking/${finalAwb}`
          : (data.order_id ? `https://shiprocket.co/tracking/order/${data.order_id}` : undefined);
        ...
        return {
          ...
          trackingUrl,
          estimatedDelivery: undefined, // May be available after pickup/scan
          shippingCost: undefined, // Calculated by Shiprocket
          providerResponse: {
            create: data,
            assign: assignResponseData,
          },
        };
      } else {
