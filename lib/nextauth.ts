...
const rawMongoUri = env.MONGODB_URI;
let normalizedMongoUri = rawMongoUri;

// Ensure the connection string has a valid scheme prefix
if (!/^mongodb(\+srv)?:\/\//i.test(rawMongoUri)) {
  // Heuristic: if it's an Atlas hostname, prefer mongodb+srv://, otherwise mongodb://
  normalizedMongoUri = /mongodb\.net/i.test(rawMongoUri)
    ? `mongodb+srv://${rawMongoUri}`
    : `mongodb://${rawMongoUri}`;
  if (process.env.NODE_ENV !== 'production') {
    console.warn('[nextauth] MONGODB_URI missing scheme. Using:', normalizedMongoUri.replace(/\/\/.*@/, '//****:****@'));
  }
}

const client = new MongoClient(normalizedMongoUri);
