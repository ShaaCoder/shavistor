...
export class CloudinaryProvider implements CloudStorageProvider {
  ...
  async upload(file: File, type: string): Promise<string> {
    ...
    const { default: cloudinary, uploadFromBuffer, getFolderForType } = await import('./cloudinary');
    ...
    const result = await uploadFromBuffer(buffer, {
      folder,
      resourceType: 'image',
      // Let Cloudinary generate a unique public_id; avoid collisions
      overwrite: false,
      tags: [type]
    });
  ...
  async delete(url: string): Promise<void> {
    ...
    const { default: cloudinary, getPublicIdFromUrl } = await import('./cloudinary');
