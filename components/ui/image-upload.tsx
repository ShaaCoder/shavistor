...
export function ImageUpload({
  ...
  const uploadImages = async () => {
    ...
    try {
      ...
      const result = await response.json() as ApiResponse<{ images: string[]; stats: any }>;
      const totalTime = Date.now() - startTime;

      if (result.success) {
        // Show success message with stats
        console.log(`✨ Upload completed in ${totalTime}ms:`, result.data?.stats);
        
        // For Cloudinary URLs, use them directly from standardized API shape
        const uploaded = result.data?.images ?? [];
        if (!Array.isArray(uploaded)) {
          throw new Error('Invalid upload response: images array missing');
        }
        ...
        throw new Error((result as any).message || (result as any).error || 'Upload failed');
      }
    } catch (err) {
